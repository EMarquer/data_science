...
def get_abstracts(author_uri, verbose=VERBOSE):
    query = SPARQ_ABSTRACTS.format(author_uri)
    sparql.setQuery(query)
    sparql.setReturnFormat(JSON)
    if verbose: print("Query:\n{}".format(query))
    ...
    for result in results['results']['bindings']:
        abstracts[result['abstract']['xml:lang']] = result['abstract']['value']
    
    if verbose: print("Abstracts");pprint(abstracts)
...
def exercise_2(author, verbose=VERBOSE):
    author_uri = get_author_uri(author, verbose=verbose)
    
    if author_uri:
        if verbose: print(author, "found:", author_uri)
    
        abstracts = get_abstracts(author_uri, verbose=verbose)

        movements = get_movements(author_uri, verbose=verbose)
        
        return author_uri, abstracts, movements
        
    else:
        if verbose: print(author, "not found")
